...
(function() {
    window.addEventListener('DOMContentLoaded', () => {
        ...
        playerContainer.style.cssText = "position: fixed; bottom: 20px; left: 20px; z-index: 9999; background: rgba(18, 18, 18, 0.9); backdrop-filter: blur(6px); border: 1px solid #dfbc63; border-radius: 30px; padding: 6px 14px; display: flex; align-items: center; gap: 10px; box-shadow: 0 4px 15px rgba(0,0,0,0.6); font-family: sans-serif;";
...
(function() {
    window.addEventListener('DOMContentLoaded', () => {
        ...
        playerContainer.innerHTML = `
            <span style="color: #dfbc63; font-size: 0.8em; font-weight: 600; letter-spacing: 0.5px;">🎧 Sunset Chill</span>
            <audio id="priveAudio" loop preload="auto">
                <!-- Pista de estilo Deep/Melodic Chill Techno similar a Lane 8 y Ben Böhmer -->
        ...
        btn.addEventListener('click', () => {
            if (audio.paused) {
                ...
                audio.play().then(() => {
                    ...
                    alert("Tocá de nuevo el play para activar la música.");
                });
            } else {
                audio.pause();
                btn.textContent = "▶";
